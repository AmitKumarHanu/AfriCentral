...
public partial class frmBillingBackData : System.Web.UI.Page
{
    SqlConnection con = new SqlConnection(CommonFunctions.conBranch);
    ...
    protected void BtnNewOpt(object sender, EventArgs e)
    {
        try
        {
            ...
            if (drpStore.SelectedIndex > 0 && txt_date.Value != String.Empty)
            {
                ...
                string qry = " Select LoginStatus,Com_Country,Currency FROM vw_UserDetails where  Com_ID like  '" + dt_login_details.Rows[0]["Com_ID"].ToString() + "%' and Br_ID like '" + drpStore.SelectedValue.ToString() + "' and  EmployeeID like '" + dt_login_details.Rows[0]["EmployeeID"].ToString() + "'";
                DataTable dtR = new DataTable();
                dtR = CommonFunctions.fetchdataBranch(qry);
                ...
                lblBranchName.Text = drpStore.SelectedItem.ToString();
            ...
            else
            {
                lblloginmsg.Attributes.Add("class", "active");
                lblloginmsg.Attributes["style"] = "color:red; font-weight:bold;";
                lblloginmsg.InnerHtml = " <strong>Warning!</strong> <h4 >" + " Please fill require details !" + " </h4>";
            }
        }
        catch (Exception ex)
        {
    ...
    protected void btnAddProduct_Click(object sender, EventArgs e)
    {
        if (txtBarCodeNo.Text != string.Empty)
        {
            ...
            string qry = " Select Quantity,  CostPrice, SalesPrice FROM tbl_ItemStock where  BarCodeNo  like  '" + txtBarCodeNo.Text.Trim() + "%' and itemRegNo='" + txtItemRegNo.Text.Trim() + "' and TicketNo='" + htnStockTicketNo.Value.Trim() + "' and " +
                "  CompID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "' and StoreID='" + drpStore.SelectedValue.ToString() + "' ";
    ...
    private void BindGrid(int rowcount)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            cmdI.Connection = con;
            cmdI.CommandText = "SP_AF_ItemQtyBk";
            ...
            cmdI.Parameters.AddWithValue("@Br_id", drpStore.SelectedValue.Trim().ToString());
    ...
    private void BindStoreDetails()
    {
        try
        {
            ...
            string qry1 = " Select X.Br_ID, X.Br_Name, X.Br_Address, X.Br_PhoneNo, X.Br_Email, X.Br_Website From  tbl_BranchMaster X, tbl_UserMaster Y where X.Br_ID=Y.Br_id and X.Com_ID=Y.Com_Id and  Y.Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "' and Y.Br_id='" + drpStore.SelectedValue.ToString() + "' ";
            DataTable dt1 = new DataTable();
            dt1 = CommonFunctions.fetchdataBranch(qry1);

            if (dt1.Rows.Count > 0)
            {
                Session["grdBrDetails"] = dt1;
                lblBrName.Text = dt1.Rows[0]["Br_Name"].ToString();
                lblBrAddress.Text = dt1.Rows[0]["Br_Address"].ToString();
                lblBrEmail.Text = dt1.Rows[0]["Br_Email"].ToString();
                lblBrPhone.Text = dt1.Rows[0]["Br_PhoneNo"].ToString();
            }
        ...
        try
        {
            ...
            if (Convert.ToDecimal(lblTotalPayment.Text) > 0)
            {
                ...
                foreach (GridViewRow r in grdAddToCard.Rows)
                {
                    ...
                    SqlCommand cmdI = new SqlCommand();
                    cmdI.Connection = con;
                    cmdI.CommandText = "SP_AF_ItemTransactionBK";
                    cmdI.CommandType = CommandType.StoredProcedure;

                    cmdI.Parameters.AddWithValue("@TicketNo", txtBillNo.Text.Trim().ToString());
                    cmdI.Parameters.AddWithValue("@TicketDate", hdnBillingDate.Value.Trim().ToString());
                    ...
                    cmdI.Parameters.AddWithValue("@EmployeeID", dt_login_details.Rows[0]["EmployeeID"].ToString());
    ...
    protected void btnPreviewReceipt_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Session["AddToCard"] != null)
            {
                ...
                SqlCommand cmdI = new SqlCommand();
                cmdI.Connection = con;
                cmdI.CommandText = "SP_AF_ItemTransactionPaymentBk";
                cmdI.CommandType = CommandType.StoredProcedure;
                cmdI.Parameters.AddWithValue("@TicketNo", txtBillNo.Text.Trim().ToString());
                cmdI.Parameters.AddWithValue("@TicketDate", hdnBillingDate.Value.Trim().ToString());
        ...
        try
        {
            ...
            if (Session["AddToCard"] != null)
            {
                ...
                if (RI == "1")
                {
                    ...
                    Response.Redirect("frmBillingBackData.aspx");
    ...
    public static List<itemResult> GetBarcode(string BarCode, string ComID, string BrID)
    {
        ...
        using (SqlConnection con = new SqlConnection(CommonFunctions.conBranch))
        {
