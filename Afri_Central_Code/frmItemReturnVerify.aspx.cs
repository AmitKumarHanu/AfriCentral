...
public partial class frmItemReturnVerify : System.Web.UI.Page
{
    ...
    public void bindgrid()
    {
        ...
        try
        {
            ...
            string qry = " Select distinct(X.Ticket), X.BranchName, X.Brand, X.Status, Convert(varchar(10), X.CreatedOn) AS CreateON From " +
            " (Select X1.TicketNo as Ticket,X1.BranchName, X1.Brand, CASE WHEN X1.isVerify = 1 THEN 'Verified' WHEN X1.isVerify = 2  THEN 'Pending' END as status, X1.CreateON as CreatedOn from AfriStoreA1.dbo.tbl_ItemTransferReturn X1, tbl_UserMaster X2 " +
            " where X1.StoreID = X2.Br_id and X1.CompID = '" + dt_login_details.Rows[0]["Com_id"].ToString() + "' and  X1.isTransfer = 1 and X1.isVerify = 2) X    group by X.Ticket, X.BranchName, X.Brand, X.status, X.CreatedOn order by CreateON desc ";
    ...
    protected void grdOrder_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            Label lblBranchName = (Label)grdOrder.SelectedRow.FindControl("lblBranchName");

            string qry = " Select  X1.TicketNo, X1.ItemRegNo, X1.ItemName, X1.ItemSpecification  , X1.Brand, X1.BarCodeNo, X1.Quantity, X1.Hold_Quantity, X1.CostPrice, X1.SalesPrice, X1.Discount, X1.DiscountAmount, X1.SupplierCode, X1.CategoryCode, X1.Mfgdate, X1.Expdate, X1.Warranty, X1.Image, X1.isTransfer, X1.isVerify , X1.BranchName   " +
             " From  AfriStoreA1.dbo.tbl_ItemTransferReturn  X1, tbl_CompanyMaster X2 where X1.CompID=X2.Com_ID  and X1.BranchName = '" + lblBranchName.Text.Trim() + "' and  X1.isTransfer=1 and X1.isVerify=2 and X1.TicketNo like '" + lblTicketNo.Text.Trim() + "%' " +
             " order by X1.TicketNo,  year(X1.CreateON) , month(X1.CreateON)  , day(X1.CreateON)  asc ";
    ...
    protected void btnVerifyReturn_Click(object sender, EventArgs e)
    {
        int Count = 0;
        foreach (GridViewRow r in grdOrderDetails.Rows)
        {
            ...
            if (ctl.Checked)
            {
                ...
                cmdI.Connection = cn1;
                cmdI.CommandText = "SP_AF_ItemReturnVerify";
