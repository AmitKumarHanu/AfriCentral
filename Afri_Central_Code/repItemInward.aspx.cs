...
public partial class repItemInward : System.Web.UI.Page
{
    ...
    public void bindgrid()
    {
        ...
        {
            //-------------Bind Invoice Master----------------
            String qryS = "  Select SupplierName, SID   From  tbl_SupplierMaster where Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "' order by SupplierName asc ";
            DataTable dtS = new DataTable();
            dtS = CommonFunctions.fetchdata(qryS);

            drpSupplier.DataSource = dtS;
            ...
            //-------------Bind Invoice Master----------------
            String qry1 = " Select  distinct(InvoiceNo), year(CreateOn)  ,month(CreateOn) From vw_ItemInward where Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "' order by year(CreateOn) desc ,month(CreateOn) ";
            DataTable dt1 = new DataTable();
            dt1 = CommonFunctions.fetchdata(qry1);

            drpStore.DataSource = dt1;
            drpStore.DataValueField = "InvoiceNo";
            drpStore.DataTextField = "InvoiceNo";
            drpStore.DataBind();
            drpStore.Items.Insert(0, new ListItem("-Invoice No.-", "0"));
            ...
            String qry3 = " Select distinct(Brand) From tbl_ItemMaster  where CompID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "'  order by Brand ";
        ...
        try
        {
            if (IsPostBack)
            {
                ...
                if (val.ToString() != String.Empty)
                {
                    ...
                    {
                        condition = " and isnull(SID,'0')   = '" + drpSupplier.SelectedValue + "'";
                        Heading = Heading + " , Supplier Name : '" + drpSupplier.SelectedItem + "'";
                    }

                    if (drpStore.SelectedIndex != 0)
                    {
                        condition = " and isnull(InvoiceNo,'0')   = '" + drpStore.SelectedValue + "'";
                        Heading = Heading + " , InvoiceNo  : '" + drpStore.SelectedItem + "'";
                    }                  
                    ...
                    if (drpCategory.SelectedValue != "0")
                    {
                        ...
                        Heading = Heading + " , Category Name : '" + drpCategory.SelectedItem + "'";
                    }
                    ...
                    if (drpBrand.SelectedIndex != 0)
                    {
                        ...
                        Heading = Heading + " , Brand Name : '" + drpBrand.SelectedItem + "'";
                    }
                    ...
                    lblRDilter.Text = Heading.ToString().ToUpper();
                    ...
                    if (Case == "1" )
                    {
                        ...
                        string qry = " Select InvoiceNo, Convert(varchar(10),CreateOn,103) as CreateOn, ItemName , ItemSpecification ,  sum(Quantity) +sum(Hold_Quantity)  As Quantity , Brand , CategoryName , SupplierName  " +
                        " from vw_ItemInward  where   Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "'   and CreateOn  between '" + dateF.ToString() + "  00:00:00.000' and '" + dateT.ToString() + " 23:59:59.000' " + condition + " "  +
                        "  group by InvoiceNo, CreateOn, ItemName, SupplierName, ItemSpecification  , SID, InvoiceNo, CategoryName, Brand " +
                        " order by  year(CreateOn)  , month(CreateOn)  , day(CreateOn), InvoiceNo ";
                    ...
                    if (Case == "2" )
                    {
                        ...
                        string qry = " Select InvoiceNo, Convert(varchar(10),CreateOn,103) as CreateOn,   ItemName,ItemSpecification,  Brand,BarCodeNo,sum(Quantity) +sum(Hold_Quantity) as Quantity  ,Hold_Quantity,CostPrice,SalesPrice, Com_Name , Brand, CID, CategoryName, SID, SupplierName, UserName, sum(Quantity)  As Quantity   " +
                            " from vw_ItemInward where   Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "'  and CreateOn  between '" + dateF.ToString() + "  00:00:00.000' and '" + dateT.ToString() + " 23:59:59.000' " + condition + " " +
                            " group by InvoiceNo,  CreateOn,  ItemName,ItemSpecification,  Brand,BarCodeNo,Quantity,Hold_Quantity,CostPrice,SalesPrice, Com_Name , Brand, CID, CategoryName, SID, SupplierName, UserName " +
                            " order by   year(CreateOn)  , month(CreateOn)  , day(CreateOn), InvoiceNo   ";
    ...
    protected void btnExcel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Session["RegReportS"] != null)
            {
                ...
                ExportToExcel(ref html, "Outword_Summary_Report");
            }
            if (Session["RegReportD"] != null)
            {
                lblFDateR.Text = lblFDate.Text;
                ...
                string html = HdnValue.Value;
                ExportToExcel(ref html, "Outword_Details_Report");
            }
