...
public partial class repStoreItemStock : System.Web.UI.Page
{
    ...
    protected void BtnSearchOpt(object sender, EventArgs e)
    {
        try
        {
            if (IsPostBack)
            {
                ...
                {
                    condition = condition + " and  Br_ID  ='" + drpStore.SelectedValue + "' ";
                ...
                if (drpStore.SelectedIndex > 0)
                {
                    string qry = " Select ItemName, ItemSpecification, Convert(varchar(10), TicketDate, 103) as TicketDate, CategoryName, Brand,    CostPrice, Quantity, (Quantity* CostPrice)  AS Amount  from AfriStoreA1.dbo.vw_ItemStockReport where Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() +
                    "'" + condition + "  order by  CategoryName, Brand  ";
