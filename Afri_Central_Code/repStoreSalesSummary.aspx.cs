...
public partial class repStoreSalesSummary : System.Web.UI.Page
{
    ...
    public void bindgrid()
    {
        ...
        try
        {
            ...
            //String qry = "SElect Upper(X.Br_Name) as Br_Name, X.Br_ID  From tbl_BranchMaster X, tbl_ClientStoreRelation Y where X.Com_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "' and X.Com_ID = Y.Com_ID and X.Br_ID=Y.Br_ID and Y.Client_ID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "'";
            ...
            drpStore.DataBind();
            drpStore.Items.Insert(0, new ListItem("--All-", "All"));
                ...
                if (val.ToString() != String.Empty)
                {
                    ...
                    if (Case == "1")
                    {
                        string qry = "SELECT TicketNo, convert(varchar(10),TicketDate,103) as TicketDate, BranchName, CustomerName,  sum(Quantity) as Quantity, CompID,StoreID,Status, (isnull(PaidAmount,0)) as PaidAmount, isnull(ChangeAmount,0) as ChangeAmount ,isnull(DueAmount,0)  as DueAmount    ,PaymentMode,PaymentReceivedBy, ReceiptedPrintedBy,isPrint  " +
                            "FROM AfriStoreA1.dbo.vw_StoreSalesSummary where CompID='" + dt_login_details.Rows[0]["Com_ID"].ToString() + "'  and TicketDate  between '" + dateF.ToString() + "  00:00:00.000' and '" + dateT.ToString() + " 23:59:59.000' " + condition + " " +
